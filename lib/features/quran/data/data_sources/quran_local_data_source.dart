...
/// Quran Local Data Source
///
/// Handles reading and parsing Quran verses from local assets and integrating with Surah models
class QuranLocalDataSource {
  ...
  /// Integrates local verses into an existing Surah model
  ///
  /// Parameters:
  /// - [surah]: The existing Surah model to update with local verses
  ///
  /// Returns: Future<SurahModel> with integrated ayahs
  Future<SurahModel> integrateVersesIntoSurah(SurahModel surah) async {
    try {
      final ayahs = await loadAyahsForSurah(surah.number);
      ...
      // If local loading fails, return the original surah with empty ayahs
      return SurahModel(
        number: surah.number,
        name: surah.name,
        englishName: surah.englishName,
